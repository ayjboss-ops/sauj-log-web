...
const REL_TRAITS = {
  ...
  earth: { 
    overview: "포용력이 넓고 듬직하여 주변에서 고민 상담을 많이 해오지만, 정작 내 속마음은 아무에게나 잘 꺼내지 않습니다.", 
...
const DAEUN_DICT = {
  ...
  produced: {
    ...
    5: { keyword: "지적 자산", desc: { mild: "풍부한 경험을 바탕으로 눈에 보이지 않는 지적 자산, 특허, 저작권 등을 통해 수익을 창출합니다. 밖으로 무리하게 뛰기보다 내실을 깊게 다져야 하는 시기입니다.", spicy: "생각만 우주 끝까지 뻗어나가고 행동은 1도 안 하는 극강의 게으름뱅이 시기입니다. 입만 털지 말고 제발 실천 하나라도 하세요." }, type: "neutral" },
...
export default function App() {
  ...
  const fetchDb = async (userId) => {
    try {
      ...
      if (docSnap.exists() && docSnap.data().profiles) {
        setSavedProfiles(docSnap.data().profiles);
      } else {
  ...
  const syncDb = async (p) => { 
    if(currentUser) await setDoc(doc(db, 'users', currentUser.id), { profiles: p }, { merge: true }); 
  };
  ...
  const handleLogout = () => { 
    setIsLoggedIn(false); 
    setCurrentUser(null); 
    localStorage.removeItem('sajuUser'); 
    setActiveTab('home'); 
  };
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      alert("이미지 저장 중 문제가 발생했습니다. (브라우저 보안 환경 확인 필요)");
    } finally {
  ...
  if (!isLoggedIn) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#0A0D14] text-white p-6 selection:bg-indigo-500/30">
      <div className="flex-1 flex flex-col items-center justify-center animate-fade-in-up py-10 w-full max-w-sm">
        ...
        <h2 className="text-2xl sm:text-3xl font-bold text-white mb-3 text-center tracking-tight font-serif">당신의 운명을 읽다,<br/>사주로그</h2>
        <p className="text-slate-400 text-center text-sm mb-10 font-medium">나만의 10년 주기 대운과<br/>오늘의 운세를 확인하세요.</p>
        
        <div className="w-full space-y-3">
          <button onClick={() => loginClick('kakao')} className="w-full bg-[#FEE500] hover:bg-[#FDD800] text-black font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-transform hover:scale-[1.02] active:scale-[0.98] shadow-md text-sm">
            <MessageCircle className="w-5 h-5" /> 카카오로 3초만에 시작하기
          </button>
          <button onClick={() => loginClick('naver')} className="w-full bg-[#03C75A] hover:bg-[#02b351] text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-transform hover:scale-[1.02] active:scale-[0.98] shadow-md text-sm">
            <MessageSquare className="w-5 h-5" /> 네이버로 3초만에 시작하기
          </button>
          
          <div className="w-full border-t border-slate-800 my-6 relative">
            <span className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[#0A0D14] px-3 text-slate-500 text-xs font-bold">또는</span>
          </div>
          
          <button onClick={guestLogin} className="w-full bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-transform hover:scale-[1.02] active:scale-[0.98] shadow-md text-sm border border-slate-700">
            <User className="w-5 h-5" /> 비회원(게스트)으로 시작하기
          </button>
  ...
  // --- 화면 렌더링 탭 ---
  const renderHomeTab = () => (
    <div className="w-full max-w-3xl mx-auto space-y-6 animate-fade-in-up mt-4 sm:mt-8 pb-12">
      <div className="bg-gradient-to-br from-indigo-900 to-slate-900 rounded-3xl p-6 sm:p-8 border border-indigo-500/30 shadow-lg relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-2xl sm:text-3xl font-bold text-white mb-2 tracking-tight">반갑습니다, {currentUser?.name}님!</h2>
  ...
  const renderMyPageTab = () => {
    ...
    const handleSaveProfile = async (e) => {
      ...
      if (editId) {
        ...
        await handleAddProfile(profileForm);
      }
      setIsFormOpen(false);
    };
  ...
  const renderSajuTab = () => {
    if (sajuStep === 'input') return (
      <div className="w-full max-w-md mx-auto p-6 bg-slate-900/80 backdrop-blur-md rounded-3xl border border-slate-700/50 mt-4 sm:mt-10 mb-24 shadow-xl animate-fade-in-up">
    ...
    return (
      <div className="w-full max-w-2xl mx-auto space-y-6 animate-fade-in-up mt-4 sm:mt-8 pb-12 print-container bg-[#0A0D14] text-slate-200">
        ...
        <div className={`pdf-solid-bg rounded-3xl p-6 sm:p-8 border text-center relative overflow-hidden shadow-sm ${flavorMode === 'spicy' ? 'border-rose-500/50 bg-[#4c0519]/20' : 'border-amber-500/30'}`}>
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-white mb-4 relative z-10 leading-snug whitespace-pre-line">{`"${displayName}님,\n${daily.total.title}"`}</h2>
      ...
      <div className="w-full max-w-3xl mx-auto space-y-6 animate-fade-in-up mt-4 sm:mt-8 pb-12 print-container bg-[#0A0D14] text-slate-200">
        ...
        <div className="pdf-solid-bg rounded-3xl p-6 sm:p-8 border border-rose-500/30 shadow-sm relative bg-[#0A0D14]">
          ...
          <div className="space-y-6">
             ...
             <div className={`p-6 rounded-2xl border shadow-lg relative overflow-hidden ${flavorMode === 'spicy' ? 'bg-[#4c0519] border-rose-500' : 'bg-slate-800 border-amber-500/50'}`}>
